...
export async function getNextORNumero() {
  ...
  const seq = last ? (parseInt(last.slice(prefix.length)) || 0) : 0;
...
export async function getOrdresSite(siteId) {
  ...
  return (data || []).map(r => ({
    ...
    immat: r.immat, typePanne: r.type_panne, technicien: r.technicien, technicien2: r.technicien2||'',
...
export async function createOrdreSite(ordre, siteId) {
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      type_panne: ordre.typePanne||ordre.type_panne||'',
      technicien: ordre.technicien||'', technicien2: ordre.technicien2||'', priorite: ordre.priorite||'normale',
...
export async function updateParcVehicule(id, v) {
  const { error } = await supabase.from('parc_vehicules').update(v).eq('id', id)
  if (error) { console.error(error); return false; }
  return true;
}
export async function deleteParcVehicule(id) {
